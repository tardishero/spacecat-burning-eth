...
import { SpacekittyNFTStaking } from "../config";
...
export default function SpaceKittyClaimpage(props: {
  nftCounts: number;
  lastClaimed: number;
  claimAmount: number;
  getData: Function;
  ...
  const handleStartOnBoarding = async () => {
    ...
    await SpaceKittyNFTStakingContract.onBoardUser()
      .then((tx: any) => {
        tx.wait()
          .then(() => {
  ...
  const handleClaimFunc = async () => {
    setStartLoadingState(true);
    await SpaceKittyNFTStakingContract.claimRewards()
      .then((tx: any) => {
        tx.wait()
          .then(() => {
            setStartLoadingState(false);
  ...
  return (
    <div className="flex flex-col gap-4 bg-white rounded-lg min-h-[45vh]">
      <div className="px-4 -mt-6">
        <div className="w-full py-4 text-center text-white rounded-lg box-shadow-custom">
          <h1 className="text-3xl text-white uppercase">spacekitty nfts</h1>
        </div>
      </div>
