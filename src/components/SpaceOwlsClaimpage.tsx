...
import { SpaceOwlsNFTStaking, SpacekittyNFTStaking } from "../config";
...
export default function SpaceOwlsClaimpage(props: {
  nftCounts: number;
  lastClaimed: number;
  claimAmount: number;
  getData: Function;
  ...
  const handleStartOnBoarding = async () => {
    ...
    await SpaceOwlsNFTStakingContract.onBoardUser()
      .then((tx: any) => {
        tx.wait()
          .then(() => {
  ...
  const handleClaimFunc = async () => {
    setStartLoadingState(true);
    await SpaceOwlsNFTStakingContract.claimRewards()
      .then((tx: any) => {
        tx.wait()
          .then(() => {
            setStartLoadingState(false);
